...
std::set<int>
gdbarch_get_capability_roots (struct gdbarch *gdbarch)
{
  gdb_assert (gdbarch != NULL);
  gdb_assert (gdbarch->get_capability_roots != NULL);
  if (gdbarch_debug >= 2)
    gdb_printf (gdb_stdlog, "gdbarch_get_capability_roots called\n");
  return gdbarch->get_capability_roots (gdbarch);
}

void
set_gdbarch_get_capability_roots (struct gdbarch *gdbarch,
