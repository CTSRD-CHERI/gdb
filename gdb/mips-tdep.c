...
mips_cheri_register_p (struct gdbarch *gdbarch, int regnum)
{
  ...
  return (cap0 != -1
	  && (regtype == register_type (gdbarch, cap0)
	      || regtype == register_type (gdbarch,
				    ...
				    struct type *type, struct value *arg2)
{
  if (TYPE_LENGTH (type) > 8)
      ...
      if (TYPE_LENGTH (type2) <= 8)
	{
	  ...
	  if (nosign)
	    store_unsigned_integer (buf + 8, 8, byte_order, longest);
	  else
	    store_signed_integer (buf + 8, 8, byte_order, longest);
	  return v;
	}
    }
  return NULL;
